<div style="margin: 20px 5px">
	...
	<input type="hidden" value="${id}" name="id" />
		<table style="font-size: 12px; border-spacing: 20px 0px">
			...
			<tr height="25px">
				<td><select id="cid" name="cid" class="easyui-combobox"
			...
			<tr height="25px">
				<td><select id="info" name="info" class="easyui-combobox" 
					data-options="editable:false,panelHeight:120,width:292,height:25">
						...
						<option value="${concession.pid}:${concession.name}:${concession.price}" <#if concession.pid == pid>selected</#if> >名称：${concession.name},价格：${concession.price / 100}</option>
						</#list>
				</select></td>
			</tr>
			...
			<tr height="25px">
				<td><select id="ptype" name="ptype" class="easyui-combobox"
					data-options="editable:false,panelHeight:92,width:292,height:25">
						<option value="0" <#if ptype == 0>selected</#if>>小食卖品</option>
						<option value="1" <#if ptype == 1>selected</#if>>充值卖品</option>
				</select></td>
			</tr>
