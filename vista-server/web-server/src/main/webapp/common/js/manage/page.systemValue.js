	...
	function init(target) {
		...
		var editFun = function(tkey, value) {
			...
			if (tkey == 1000) {
				...
				title = "消息推送";
				url = opt.appUrl + "m_systemValue/toEdit_push";
				height = 400;
				width = 370;
			} else {
				return;
				...
				onLoad : function() {
					if (editDialog.find('#theform').length > 0) {
						...
						if (url == opt.appUrl + "m_systemValue/toEdit_html") {
							editDialog.dialog("options").editor = KindEditor
									.create(editDialog.find('#value'), {
										width : editDialog.find('.tabs-panels')
												.width() - 60,
										height : 500,
										minHeight : 200,
										resizeType : 0,
										uploadJson : opt.appUrl
												+ "m_goods/uploadimg", // 指定上传文件的服务器端程序
				...
				.datagrid(
						{
							...
							columns : [ [
									...
									},
									{
										...
										formatter : function(val, row) {
											if (row.key != 9999) {
												return "<a class='btn_edit' data-key='"
														+ row.key
														+ "' data-value='"
														+ row.value
														+ "'>编辑</a>";
